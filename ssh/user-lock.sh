...
RED="\033[0;31m"
BURIQ () {
curl -sS https://dpvpn.berkahost.my.id/sc/register > /root/tmp
...
Name=$(curl -sS https://dpvpn.berkahost.my.id/sc/register | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
MYIP=$(curl -sS ipv4.icanhazip.com)
IZIN=$(curl -sS https://dpvpn.berkahost.my.id/sc/register | awk '{print $4}' | grep $MYIP)
if [ "$MYIP" = "$IZIN" ]; then
Bloman
else
